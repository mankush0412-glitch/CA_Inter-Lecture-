...
// everywhere it was imported. The ready promise is important: without waiting
// for this initial read, a fast admin request could start Auto Mode and then
// have its fresh in-memory state overwritten by the old Mongo document.
...
const autoLectureSessionReady = (async () => {
  try {
...
// Auto Mode can receive several Telegram updates while the previous video is
// still being stored. Serialize the Mongo batch write and the session counter
// update together so two videos can never receive the same lecture number or
// leave the persisted session behind the actual batch.
let autoLectureLock = Promise.resolve();
function withAutoLectureLock(fn) {
  const run = autoLectureLock.then(fn, fn);
  autoLectureLock = run.catch(() => {});
  return run;
}
...
async function autoSaveLecture({ link }) {
  await autoLectureSessionReady;
  return withAutoLectureLock(async () => {
    if (!autoLectureSession.active) return null;
    const lectureNumber = autoLectureSession.lectureCount + 1;
    ...
    autoLectureSession.lectureCount = lectureNumber;
    await _saveAutoSession();
    return { lectureNumber, lectureName: `Lecture ${lectureNumber}` };
  });
}
...
router.get('/batch-referral/status/:userId/:batchId', async (req, res) => {
  try {
    ...
    const unlockDurationHours = (batch && batch.unlockDurationHours) || 168;

    let doc = await BatchReferralUnlock.findOne({ userId, batchId }).lean();
    if (!doc) return res.json({ unlocked: false, referralCount: 0, required, unlockDurationHours, expiresAt: null, botUsername: BOT_USERNAME });
    ...
    res.json({ unlocked, referralCount, required, unlockDurationHours, expiresAt, botUsername: BOT_USERNAME });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/spin/token/:userId', async (req, res) => {
  try {
    ...
    if (!status.canSpin) {
      if (status.spinsLeft <= 0) return res.status(429).json({ error: `Aaj ke saare ${SPIN_DAILY_LIMIT} spins ho gaye! Kal wapas aao.`, ...status });
  ...
  try {
    ...
    const result = await withUserLock(userId, async () => {
      ...
      if (!status.canSpin) {
        ...
        if (status.spinsLeft <= 0) return { error: { status: 429, body: { error: `Aaj ke saare ${SPIN_DAILY_LIMIT} spins ho gaye! Kal wapas aao.`, ...status } } };
...
router.post('/auto-lecture/start', verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    const session = await withAutoLectureLock(async () => {
...
router.post('/auto-lecture/stop', verifyAdmin, async (req, res) => {
  try {
    await autoLectureSessionReady;
    const totalAdded = await withAutoLectureLock(async () => {
      const count = autoLectureSession.lectureCount;
...
router.autoSaveLecture = autoSaveLecture;
router.saveAutoSession = _saveAutoSession;
router.autoLectureSessionReady = autoLectureSessionReady;
