...
const autoSaveLecture = courseRoutes.autoSaveLecture;
...
async function startBot() {
  // Load the persisted Auto Mode session before consuming Telegram updates.
  // Otherwise an update arriving during startup can be handled with the
  // default inactive state.
  if (courseRoutes.autoLectureSessionReady) await courseRoutes.autoLectureSessionReady;
    ...
    enqueueFile(msg.from.id, async () => {
      ...
      try {
        ...
        if(autoLectureSession&&autoLectureSession.active){
          try{
            const saved = await autoSaveLecture({ link: code });
            if (!saved) throw new Error('Auto Mode is no longer active');
            const lNum = saved.lectureNumber; const lName = saved.lectureName;
