...
async function startBot() {
  ...
  bot.onText(TG_LINK_RE, (msg,match) => {
    ...
    enqueueFile(msg.from.id, async () => {
      ...
      try {
        ...
        await bot.deleteMessage(chatId,processing.message_id);
        // A t.me source link is always the normal/manual save flow. Auto Mode
        // is reserved for sending a video directly to the bot below.
  ...
  bot.on("message", (msg) => {
    ...
    if(!fileInfo) return;
    const isForwardedMessage=!!(msg.forward_origin||msg.forward_from||msg.forward_from_chat);
    const session=bulkSessions.get(userId);
    ...
    enqueueFile(userId, async () => {
      ...
      try {
        ...
        const link=`https://t.me/${BOT_USERNAME}?start=${code}`;
        await bot.deleteMessage(chatId,processing.message_id);
        // A Telegram-forwarded video is a normal/manual save. Auto Mode only
        // handles a video sent directly to the bot (without forward metadata).
        if(autoLectureSession&&autoLectureSession.active&&!isForwardedMessage){
          try{
