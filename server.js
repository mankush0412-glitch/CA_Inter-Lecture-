...
// Telegram only sends chat_member leave updates when the bot is an admin in
// the force-join chat. Keep a small membership watchdog for delivered videos
// as a fallback, so a missed update cannot leave the lecture in the user's DM.
const forceJoinMonitorTimers = new Map();
const FORCE_JOIN_RECHECK_MS = 15 * 1000;

function getAllForceJoinChannels() {
  return [...new Set([...REFERRAL_FORCE_JOIN_CHANNELS, ...FORCE_JOIN_CHANNELS])];
}

function isAbsentFromChat(member) {
  return !member
    || member.status === "left"
...
async function hasJoinedAllForceJoinChats(botInstance, userId) {
  const channels = getAllForceJoinChannels();
  if (!channels.length) return true;

  for (const channel of channels) {
    try {
      const member = await botInstance.getChatMember(channel, userId);
      if (isAbsentFromChat(member)) return false;
    } catch (err) {
      // Do not delete a video on a temporary Telegram/API error. The next
      // watchdog pass will retry, and the chat_member handler remains active.
      console.warn(`Force-join membership recheck failed for ${channel}:`, err.message);
    }
  }
  return true;
}

async function deletePendingVideosAfterLeave(userId) {
  ...
  for (const pending of pendingVideos) {
    try { await bot.deleteMessage(userId, pending.message_id); } catch (_) {}
  }
...
async function handleForceJoinLeave(userId) {
  await revokeAllBatchPremiumForUser(userId).catch(() => {});
  await deletePendingVideosAfterLeave(userId);
}

function startForceJoinMonitor(userId) {
  if (!getAllForceJoinChannels().length || forceJoinMonitorTimers.has(userId)) return;

  const checkMembership = async () => {
    forceJoinMonitorTimers.delete(userId);
    const hasPendingVideo = await PendingDelete.exists({ chat_id: userId }).catch(() => false);
    if (!hasPendingVideo) return;

    const stillJoined = await hasJoinedAllForceJoinChats(bot, userId);
    if (!stillJoined) {
      await handleForceJoinLeave(userId);
      return;
    }

    const timer = setTimeout(checkMembership, FORCE_JOIN_RECHECK_MS);
    forceJoinMonitorTimers.set(userId, timer);
  };
...
async function startBot() {
      ...
      if (param.startsWith("B")) {
        try {
          ...
          for (const f of batch.files) {
            ...
            if ((f.file_type==="video"||f.file_type==="video_note") && sentMsg) {
              hasVideo=true;
              scheduleDelete(bot,chatId,sentMsg.message_id,new Date(Date.now()+6*60*60*1000));
              startForceJoinMonitor(chatId);
            }
          }
  ...
  // A forwarded media message can reach both the text-link and generic message
  // listeners in quick succession. Claim the Telegram message once so it
  // cannot create both a normal-save response and an auto-save response.
  const handledInboundMessages=new Map();
  const INBOUND_MESSAGE_TTL_MS=5*60*1000;
  function claimInboundMessage(msg){
    const key=msg?.chat?.id!=null&&msg?.message_id!=null?`${msg.chat.id}:${msg.message_id}`:null;
    if(!key) return true;
    const now=Date.now();
    for(const [oldKey,claimedAt] of handledInboundMessages){
      if(now-claimedAt>INBOUND_MESSAGE_TTL_MS) handledInboundMessages.delete(oldKey);
    }
    if(handledInboundMessages.has(key)) return false;
    handledInboundMessages.set(key,now);
    return true;
  }
  ...
  bot.onText(TG_LINK_RE, (msg,match) => {
    if(isGroupChat(msg)||!isOwner(msg.from?.id)) return;
    if(!claimInboundMessage(msg)) return;
  ...
  bot.on("chat_member", async (update) => {
    ...
    const allForceJoinChannels = getAllForceJoinChannels();
